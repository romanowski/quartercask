...
private[quartercask] class AWSIAM(region: Region) {

  val BasicLambdaRoleName = "lambda_basic_execution"

  lazy val iamClient = buildIamClient
  ...
  private def buildIamClient: AmazonIdentityManagement = {
    val builder = AmazonIdentityManagementClientBuilder.standard()
